...
Hummingbird.prototype = {
  ...
  addEnabledMetrics: function(db) {
    var files = fs.readdirSync(path.join(__dirname, 'metrics', 'enabled'));

    for(var i = 0; i < files.length; i++) {
      var file = files[i];
      var filename = file.replace(/\.js$/, '');

      var m = require(path.join(__dirname, 'metrics', 'enabled', filename));
